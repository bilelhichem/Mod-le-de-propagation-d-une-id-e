...
class EnhancedPDSimulator:
    def __init__(self, master):
        self.master = master
        self.master.title("Spatial Prisoner's Dilemma Simulator")
        
        # Paramètres de simulation
        self.n = 100
        self.b = 1.6  # Avantage initial des défecteurs
        self.running = False
        self.neighborhood = 'Moore'
        self.boundary = 'periodic'
        self.initial_config = 'random'
        self.strategy_mode = 'local'
        self.prev_grid = None

        # Configuration visuelle
        self.cmap = ListedColormap(['red', 'blue', 'yellow', 'green'])
        self.colors = {0: 0, 1: 1, 2: 2, 3: 3}
        self.ts_data = []
    ...
    def initialize_grid(self):
        if self.initial_config == 'random':
            return np.random.choice([0, 1], size=(self.n, self.n), p=[0.2, 0.8])
        elif self.initial_config == 'single_d':
            grid = np.ones((self.n, self.n))
            grid[self.n//2-3:self.n//2+3, self.n//2-3:self.n//2+3] = 0
            return grid
    ...
    def setup_controls(self):
        ...
        tk.Button(control_frame, text="Démarrer", command=self.toggle_simulation).pack(side=tk.LEFT, padx=5)
        tk.Button(control_frame, text="Réinitialiser", command=self.reset_grid).pack(side=tk.LEFT, padx=5)
        tk.Button(control_frame, text="Bords", command=self.toggle_boundary).pack(side=tk.LEFT, padx=5)

        strategy_frame = tk.LabelFrame(control_frame, text="Stratégies", padx=5, pady=2)
        strategy_frame.pack(side=tk.LEFT, padx=5)

        strategy_options = [
            ("Défection Pure", 'always_d'),
    ...
    def setup_visualization(self):
        ...
        self.ts_ax.set_xlabel("Génération")
        self.ts_ax.set_ylabel("Part de Coopérateurs")
        self.ts_ax.set_ylim(0, 1)
        self.ts_line, = self.ts_ax.plot([], [], 'b-')
        self.ts_ax.axhline(0.318, color='r', linestyle='--', label="Limite théorique")
        self.ts_ax.legend()
        ...
        for i in range(self.n):
            for j in range(self.n):
                if self.strategy_mode == 'always_d':
                    ...
                    new_grid[i, j] = 1
                elif self.strategy_mode == 'tit_for_tat':
                    defector_count = 0
                    for di in (-1, 0, 1):
                        for dj in (-1, 0, 1):
                            if di == 0 and dj == 0:
                            ...
                            if 0 <= ii < self.n and 0 <= jj < self.n:
                                if self.prev_grid[ii, jj] == 0:
                                    defector_count += 1
                    new_grid[i, j] = 0 if defector_count >= 3 else 1  # Seuil ajusté à 3
                else:
                    nbrs = []
                    for di in (-1, 0, 1):
                        for dj in (-1, 0, 1):
    ...
    def update_plot(self):
        ...
        if self.prev_grid is not None:
            transition_grid = np.where(
                (self.grid == 0) & (self.prev_grid == 1), 2,
        ...
        stats_text = (f"Taille moyenne: {cluster_stats['avg_c_size']:.1f}\n"
                      f"Taille max: {cluster_stats['max_c_size']}\n"
                      f"Nombre: {cluster_stats['n_clusters']}")
        self.grid_ax.text(
