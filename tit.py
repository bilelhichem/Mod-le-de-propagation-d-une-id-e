...
class EnhancedPDSimulator:
    ...
    def setup_controls(self):
        ...
        start_btn = tk.Button(control_frame, text="Démarrer", command=self.toggle_simulation)
        start_btn.pack(side=tk.LEFT, padx=5)

        reset_btn = tk.Button(control_frame, text="Réinitialiser", command=self.reset_grid)
        reset_btn.pack(side=tk.LEFT, padx=5)

        boundary_btn = tk.Button(control_frame, text="Bords", command=self.toggle_boundary)
                ...
                if self.strategy_mode == 'always_d':
                    ...
                    for di in (-1, 0, 1):
                        for dj in (-1, 0, 1):
                            if di == 0 and dj == 0:
                                continue
                            ni = (i + di) % self.n if self.boundary == 'periodic' else i + di
                            nj = (j + dj) % self.n if self.boundary == 'periodic' else j + dj
                            if 0 <= ni < self.n and 0 <= nj < self.n:
                                if self.prev_grid[ni, nj] == 1:
                                    coop_count += 1
                                else:
                                    defect_count += 1
                    new_grid[i, j] = 1 if coop_count > defect_count else 0
                else:
                    nbrs = []
                    for di in (-1, 0, 1):
                        for dj in (-1, 0, 1):
